...
import { initialState, ActionType } from "./mailReducer.types";
type MailReducerProps = (
  state: initialState,
  action: ActionType
) => initialState;

export const mailReducer: MailReducerProps = (state, action) => {
  switch (action.type) {
    case "STAR_MAIL":
      return {
        ...state,
    ...
    case "REMOVE_FROM_SPAM":
      return {
        ...
        spam: state.spam.filter((mail: Mail) => mail.mId !== action.payload.mId)
      };
